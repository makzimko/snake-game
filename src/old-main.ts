...
const drawSnake = () => {
    ...
    for (let i = 0; i < snake.body.length; i++) {
        const correctX = getCorrectPosition(x, size.width);
        const correctY = getCorrectPosition(y, size.height);

        document.querySelector(`#cell-${correctY}-${correctX}`)!.classList.add('snake');
